...
export default function NewsletterTalente() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/newsletter', {
        ...
        body: JSON.stringify({ email, source: 'talente' }),
      });
        ...
        <form onSubmit={handleSubmit} className="max-w-xl mx-auto fade-in fade-in-delay-200">
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 px-6 py-4 border-2 border-gray-300 rounded-full text-gray-900 text-lg focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20 disabled:opacity-50 transition-all duration-300 hover:border-gray-400"
            />
            <button
              type="submit"
              disabled={status === 'loading'}
              className="px-8 py-4 bg-emerald-500 text-white font-bold rounded-full hover:bg-emerald-600 hover:shadow-xl hover:shadow-emerald-500/30 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 whitespace-nowrap"
            >
