...
export default function NewsletterUnternehmen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/newsletter', {
        ...
        body: JSON.stringify({ email, source: 'unternehmen' }),
      });
    ...
    <section id="newsletter" className="relative py-16 lg:py-24 px-6 lg:px-12 bg-white overflow-hidden">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <p className="text-lg text-gray-600 mb-8 fade-in fade-in-delay-100">
          Sei eines der ersten Unternehmen und finde Talente, die wirklich passen.
        </p>
        ...
        <form onSubmit={handleSubmit} className="max-w-xl mx-auto fade-in fade-in-delay-200">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              type="submit"
              disabled={status === 'loading'}
              className="px-8 py-4 bg-blue-500 text-white font-bold rounded-full hover:bg-blue-600 hover:shadow-xl hover:shadow-blue-500/30 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 whitespace-nowrap"
            >
